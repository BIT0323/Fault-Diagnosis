...
def preprocess_data_fusion(combined_df, signal_length, Feature_Dimension,
                           TEST_SIZE, RANDOM_SEED, normalize_raw=True, normalize_feat=True):
    """
    :function
        为特征融合模型（原始信号 + 手工特征）准备数据
        自动切分样本、提取手工特征、标准化、编码标签，并返回训练/测试集及标准化器
    :param
        combined_df: pandas DataFrame，必须包含 'fault_type' 和 'condition' 列
        signal_length: 每个样本的时序长度（如 512）
        Feature_Dimension: 1 或 3，选择使用多少维振动信号
        TEST_SIZE: 测试集比例（如 0.2）
        RANDOM_SEED: 随机种子，用于可复现划分
        normalize_raw: bool，是否对原始信号进行标准化（默认 True）
        normalize_feat: bool，是否对手工特征进行标准化（默认 True）
    :return
        X_train_raw_scaled: numpy array，标准化后的原始信号（训练集）
        X_test_raw_scaled: numpy array，标准化后的原始信号（测试集）
        X_train_feat_scaled: numpy array，标准化后的手工特征（训练集）
        X_test_feat_scaled: numpy array，标准化后的手工特征（测试集）
        y_train: numpy array，整数编码标签（训练集）
        y_test: numpy array，整数编码标签（测试集）
        le: LabelEncoder 对象，用于标签解码
        num_classes: int，类别总数
        cond_train: numpy array，工况标签（训练集）
        cond_test: numpy array，工况标签（测试集）
        scaler_raw: StandardScaler 对象，用于标准化原始信号（可保存复用）
        scaler_feat: StandardScaler 对象，用于标准化手工特征（可保存复用）
    """
    ...
    # ===== 2. 按 (故障, 工况) 分组切分样本 =====
    X_raw = []      # 原始信号 (样本数, signal_length, Feature_Dimension)
    y = []          # 故障类型字符串（原始标签）
    cond = []       # 工况字符串
    ...
    grouped = combined_df.groupby(['fault_type', 'condition'])
    print("检测到的分组:", list(grouped.groups.keys()))

    for (fault, cond_name), group in grouped:
        signal = group[feature_cols].values
        num_samples = len(signal) // signal_length
        for i in range(num_samples):
            seq = signal[i * signal_length : (i + 1) * signal_length]
    ...
    X_raw = np.array(X_raw)          # (样本数, 512, Feature_Dimension)
    y = np.array(y)
    cond = np.array(cond)
    ...
    # ===== 4. 划分训练集和测试集 =====
    X_train_raw, X_test_raw, y_train, y_test, cond_train, cond_test = train_test_split(
        X_raw, y_encoded, cond, test_size=TEST_SIZE,
        random_state=RANDOM_SEED, stratify=y_encoded
    )
    ...
    # ===== 5. 提取手工特征 =====
    print("提取手工特征...")
    X_train_feat = extract_handcrafted_features_from_samples(X_train_raw)
    X_test_feat = extract_handcrafted_features_from_samples(X_test_raw)
    feat_dim = X_train_feat.shape[1]
    print(f"手工特征维度: {feat_dim}")
    ...
    if normalize_raw:
        ...
        train_flat = X_train_raw.reshape(-1, Feature_Dimension)
        train_scaled = scaler_raw.fit_transform(train_flat)
        X_train_raw_scaled = train_scaled.reshape(X_train_raw.shape)

        test_flat = X_test_raw.reshape(-1, Feature_Dimension)
        test_scaled = scaler_raw.transform(test_flat)
        X_test_raw_scaled = test_scaled.reshape(X_test_raw.shape)
    else:
        X_train_raw_scaled = X_train_raw
        X_test_raw_scaled = X_test_raw
    ...
    if normalize_feat:
        ...
        X_test_feat_scaled = X_test_feat

    return (X_train_raw_scaled, X_test_raw_scaled,
            X_train_feat_scaled, X_test_feat_scaled,
            y_train, y_test,
            le, num_classes,
            cond_train, cond_test,
            scaler_raw, scaler_feat)
